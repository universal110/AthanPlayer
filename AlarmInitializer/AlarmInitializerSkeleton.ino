...
// ADD HERE: How many Prayers are there?
#define PRAYER_COUNT _____      
#define SD_CS_PIN    10
...
void setup() {
  ...
  // — audio
  audio.speakerPin = 9; 
  audio.setVolume(_____);   // ADD HERE: Set the volume (0-7)
...
void loop() {
  ...
  if (!inPrayer) {
    for (int i = 0; i < PRAYER_COUNT; i++) {
      if (hour == _____ && minute == _____) { // ADD HERE: Check if current hour and minute matches a prayer time -  Where did we store these values?(scroll up)
        inPrayer    = true;
        prayerEnd   = millis() + 5UL * 60UL * 1000UL;  // 5 minutes
        currentName = prayers[i];
        Serial.print("Time for "); Serial.println(currentName);
        if (SD.exists(AZAN_FILE)) audio.play(________); // ADD HERE: Play the Azan file
        break;
      }
    }
  }
  ...
  if (inPrayer && millis() < prayerEnd) {
    ...
    inPrayer = false;
    char dateBuf[11];
    sprintf(dateBuf, "_____/_____/20____", month, date, year); // ADD HERE: Display the date in MM/DD/YYYY format
    lcd.print(dateBuf);
  }

  delay(____); // ADD HERE: Wait almost 1 second before repeating the loop - ensure it's not over 1000 (1000 ms = 1s)
}
